...
def sendspeeddata() -> None:
    ...
    signature("centrala.messagehandler").apply_async(
        kwargs={"data": data}, queue="centrala"
    )
...
def setup_periodic_tasks(sender, **kwargs):
    sender.add_periodic_task(
        POCIAG_DELAY,
        signature("pociag.sendspeeddata", args=(), options={"queue": "pociag"}),
    )
    sender.add_periodic_task(
        POCIAG_DELAY_2,
        signature("pociag.sendstationdata", args=(), options={"queue": "pociag"}),
    )
